...
// --- Runtime state structs (extracted from Battle so BattleContext can reference them) ---
...
public:
    ...
    // 获取运行时状态（供技能规则访问）
    RuntimeMoveState& getRuntimeMoveState() { return runtimeMoveState; }
    const RuntimeMoveState& getRuntimeMoveState() const { return runtimeMoveState; }

    // 获取战斗上下文（供技能、特性、道具系统解耦访问）
    BattleContext& getContext() { return battleContext; }
    const BattleContext& getContext() const { return battleContext; }
