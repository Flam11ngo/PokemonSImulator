...
void resetSpecialEventFile(Battle& battle) {
    battle.clearSpecialEvents();
}

void recordSpecialEvent(const Battle& battle, const std::string& eventType, const json& details) {
...
bool applyConfusionMove(Battle& battle, Pokemon* defender, const Move& move) {
    ...
    recordSpecialEvent(battle, "status_apply", {
...
float Battle::adjustedTypeEffectivenessForMove(Pokemon* defender, Type moveType, AbilityType attackerAbility) const {
    if (!defender) {
        return 1.0f;
    }

    auto adjustedMultiplierForType = [this, moveType, defender, attackerAbility](Type defendingType) {
        ...
        if (defendingType == Type::Ghost
            ...
            if (abilityOverridesGhostImmunity(attackerAbility)) {
                multiplier = 1.0f;
            } else {
...
SemiInvulnerableState Battle::getSemiInvulnerableState(const Pokemon* pokemon) const {
...
void Battle::applyStatusEffects() {
    for (Side* side : {&sideA, &sideB}) {
        ...
        if (!active || active->isFainted()) {
            ...
            side->tickMagicCoatTurns();
            side->tickSnatchTurns();
            continue;
        }
...
void Battle::endTurn() {
    ...
    for (auto it = runtimeMoveState.cudChewPending.begin(); it != runtimeMoveState.cudChewPending.end();) {
        ...
        if (!owner || owner->isFainted() || !abilityHasCudChew(owner->getAbility()) || owner->getItemType() != ItemType::None) {
        ...
        if (state.dueTurn <= turnNumber) {
            ...
            recordSpecialEvent(*this, "item_replay", {
...
const std::vector<nlohmann::json>& Battle::getSpecialEvents() const {
    return specialEvents;
}

void Battle::clearSpecialEvents() {
    specialEvents.clear();
}

void Battle::appendSpecialEvent(const std::string& eventType, const nlohmann::json& details) {
    nlohmann::json event;
...
void Battle::enqueueAction(const BattleAction& action) {
    ...
    if (adjusted.type == ActionType::Attack && adjusted.actor) {
        tryQuickClawActivation(adjusted.actor->getItemType(), adjusted.priority);
    }
    // Tailwind: double speed for the actor's side
    if (adjusted.actor) {
...
void Battle::processTurn() {
    ...
    BattleToJson::writeToCache(BattleToJson::battleAllInfoToJson(*this), "output_" + std::to_string(turnNumber) + ".json");
}
    ...
    switch (action.type) {
        case ActionType::Attack: {
            ...
            if (multiHitConfig.has_value()) {
                ...
                int landedHits = 0;
                const bool hasLoadedDice = itemMaximizesMultiHit(action.actor->getItemType())
                    && multiHitConfig->minHits < multiHitConfig->maxHits;
                const int maxHits = hasLoadedDice ? multiHitConfig->maxHits : rollHitCount(*multiHitConfig);
                ...
                for (int hitIndex = 0; hitIndex < maxHits; ++hitIndex) {
                    ...
                    const bool preventsContact = itemPreventsContact(action.actor->getItemType());

                    damageContext.isContact = selectedMove.getCategory() == Category::Physical && !preventsContact;
                    ...
                    abilityDamageContext.context = &battleContext;
            ...
            const bool preventsContact2 = itemPreventsContact(action.actor->getItemType());

            damageContext.isContact = selectedMove.getCategory() == Category::Physical && !preventsContact2;
        ...
        case ActionType::UseItem: {
            ...
            if (isItemUsageBlockedByEmbargo(action.actor)) {
                recordSpecialEvent(*this, "item_blocked", {
            ...
            if (held.type == action.item) {
                ...
                held.executeTrigger(ItemTrigger::OnEat, action.actor, action.target, battleContext);
...
int Battle::calculateDamage(Pokemon* attacker, Pokemon* defender, const Move& move) const {
    ...
    damage *= abilityOutgoingDamageMultiplier(
        ...
        faintedAllies);
    damage *= knockOffDamageMultiplier(move, defender->getItemType());
...
bool Battle::switchPokemon(Side& side, int newIndex) {
    ...
    if (switched) {
        recordSpecialEvent(*this, "switch", {
            {"side", side.getName()},
...
void Battle::applyEntryHazardsOnSwitchIn(Side* enteringSide, Pokemon* enteringPokemon) {
    ...
    if (itemBlocksEntryHazards(enteringPokemon->getItemType())) {
        return;
    }
        ...
        if (!poisonType && !steelType) {
            ...
            for (StatusType status : afterStatuses) {
                if (!hasStatusInSnapshot(beforeStatuses, status)) {
                    recordSpecialEvent(*this, "status_apply", {
...
void Battle::processMoveEffects(Pokemon* attacker, Pokemon* defender, const Move& move) {
    ...
    if (itemBlocksSecondaryEffects(defender->getItemType())) {
        return;
    }
    ...
    if (moveNameEquals(move, "encore")) {
        ...
        if (it != runtimeMoveState.lastUsedMoveName.end() && !it->second.empty()) {
            runtimeMoveState.encoreState[defender] = EncoreState{it->second, 3};
        }
        return;
    }

    if (tryKnockOffItemRemoval(move, defender)) {
        return;
    }
...
void Battle::triggerAbility(Pokemon* pokemon, Trigger trigger, Pokemon* opponent, void* context) {
    ...
    const Ability& ability = registry.getAbility(abilityType);

    if (ability.hasTrigger(trigger)) {
        void* triggerContext = context;
        if (!triggerContext && (trigger == Trigger::OnEntry || trigger == Trigger::OnExit)) {
            triggerContext = &battleContext;
        }
        recordSpecialEvent(*this, "ability_trigger", {
...
void Battle::triggerItemEffect(Pokemon* pokemon, ItemTrigger trigger, Pokemon* opponent, void* context) {
    ...
    const Item& item = registry.getItem(pokemon->getItemType());
    if (!item.hasTrigger(trigger)) {
        return;
    }

    recordSpecialEvent(*this, "item_trigger", {
        {"pokemon", pokemon->getName()},
    ...
    item.executeTrigger(trigger, pokemon, opponent, battleContext, context);
    const ItemType afterItem = pokemon->getItemType();

    if (abilityHasCudChew(pokemon->getAbility())
