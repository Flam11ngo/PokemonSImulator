...
Pokemon::Pokemon(const Species& species, Nature nat, AbilityType abil, bool isHiddenAbil,
    ...
    : species(species), name(species.name),
      ...
      type2(species.type2),
        ...
        ivs(ivs), evs(evs), statStages{0, 0, 0, 0, 0, 0, 0}, isProtected(false), substituteHP(0), leechSeedSource(nullptr), accuracyStage(0), evasionStage(0), stockpileCount(0), consumedItemType(ItemType::None), paradoxActive(false) {
    recalculateStats();
    currentHP = maxHP;
}
...
void Pokemon::addStatus(StatusType s, int duration) {
    ...
    if (s == StatusType::Poison || s == StatusType::ToxicPoison) {
        const bool isSteelType = type1 == Type::Steel || type2 == Type::Steel;
        const bool isPoisonType = type1 == Type::Poison || type2 == Type::Poison;
        if (isSteelType || isPoisonType) {
            return;
        }
    }
...
void Pokemon::addStatus(StatusType s, int duration, AbilityType inflictorAbility) {
    if (ability == AbilityType::Immunity && (s == StatusType::Poison || s == StatusType::ToxicPoison)) {
        return;
    }
    ...
    if (s == StatusType::Poison || s == StatusType::ToxicPoison) {
        const bool isSteelType = type1 == Type::Steel || type2 == Type::Steel;
        const bool isPoisonType = type1 == Type::Poison || type2 == Type::Poison;
        if ((isSteelType || isPoisonType) && !abilityOverridesPoisonTypeImmunity(inflictorAbility)) {
            return;
        }
    }
...
void Pokemon::changeStatStage(StatIndex index, int delta) {
    ...
    if (delta < 0
        && (abilityBlocksGenericStatDrops(ability)
            || itemType == ItemType::ClearAmulet
            || (index == StatIndex::Attack && abilityBlocksAttackDrops(ability)))) {
        return;
    }

    if (abilityReversesStatChanges(ability)) {
        delta = -delta;
    }
...
void Pokemon::resetStatStages() {
    for (size_t i = 0; i < statStages.size(); ++i) {
        statStages[i] = 0;
    }
    accuracyStage = 0;
    evasionStage = 0;
...
void Pokemon::changeAccuracyStage(int delta) {
    if (delta < 0 && abilityBlocksAccuracyDrops(ability)) {
        return;
    }
    accuracyStage += delta;
    if (accuracyStage > 6) {
...
void Pokemon::changeEvasionStage(int delta) {
    if (delta < 0 && abilityBlocksEvasionDrops(ability)) {
        return;
    }
    evasionStage += delta;
    if (evasionStage > 6) {
...
Pokemon::Pokemon(const Pokemon& other) :
    ...
    stockpileCount(other.stockpileCount),
    consumedItemType(other.consumedItemType),
    paradoxActive(other.paradoxActive) {
}
