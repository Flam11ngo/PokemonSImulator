...
void applyStandardMoveEffect(BattleContext& ctx, Pokemon* attacker, Pokemon* defender, const Move& move) {
    ...
    if (move.getName() == "Dragon Dance") {
        ...
        ctx.triggerItemEffect(attacker, ItemTrigger::OnStatChange, defender);
        return;
    }
    ...
    switch (effect) {
        ...
        case MoveEffect::Drain: {
            ...
            if (drainPercentage > 0) {
                int damageDealt = ctx.calculateDamage(attacker, defender, move);
        ...
        case MoveEffect::StatChange: {
            ...
            if (!affectSelf && changeAmount < 0) {
                Side* targetSide = ctx.findSideForPokemon( target);
            ...
            if (!affectSelf && changeAmount < 0) {
                if (itemPreventsStatDrops(target->getItemType())) {
                    break;
                }
            ...
            if (afterStage != beforeStage) {
                ctx.triggerItemEffect(target, ItemTrigger::OnStatChange, itemOpponent);
            }
            break;
        }
